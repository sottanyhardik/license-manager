// LicensePlanningPanel — port from legacy/frontend/src/components/planning/LicensePlanningPanel.tsx
// Dialog that groups a license's import items by description, lets the user plan
// utilization splits (3-way auto-calc: qty ↔ unit_price ↔ cif_fc) and bulk-saves.
...
import { Loader2, Plus, Save, Target, Trash2, Wand2, X } from 'lucide-react'
...
let _sk = 0
const nextKey = () => `s${++_sk}`
const num = (v: string | number | undefined) => parseFloat(String(v ?? '')) || 0
...
// ─── License / item-plan API helpers ──────────────────────────────────────────

async function fetchLicense(licenseId: number) {
  const { data } = await apiClient.get(ENDPOINTS.LICENSES.DETAIL(licenseId))
  return data
}

async function fetchItemPlans(licenseId: number) {
  const { data } = await apiClient.get(ENDPOINTS.LICENSES.ITEM_PLANS(licenseId))
  return data
}

interface BulkUpsertLine {
  import_item: number
...
async function bulkUpsertItemPlans(licenseId: number, lines: BulkUpsertLine[]) {
  const { data } = await apiClient.post(
    ENDPOINTS.LICENSES.ITEM_PLANS_BULK_UPSERT(licenseId),
    { plans: lines },
  )
  return data
}

async function fetchNormPrefill(licenseId: number) {
  const { data } = await apiClient.get(ENDPOINTS.LICENSES.NORM_PREFILL(licenseId))
  return data as { norm: string | null; plan: Record<string, { planned_quantity: number; planned_cif: number }> | null }
}
...
export default function LicensePlanningPanel({
    ...
    void (async () => {
      ...
      try {
        ...
        const planList: Array<{
          ...
          unit_price?: number
          planned_cif_fc?: number
        }> = Array.isArray(plans) ? plans : (plans?.results ?? plans?.data ?? [])

        const splitsByItem: Record<number, SplitRow[]> = {}
        planList.forEach((p) => {
        ...
        setPoolBalance(
          Number(license?.get_balance_cif ?? license?.balance_cif ?? balanceCif ?? 0),
        )
        ...
        const itemList: Array<{
          ...
          hs_code_detail?: { hs_code: string }
        }> =
          license?.import_license ??
          license?.import_license_read ??
          []
        ...
        const byKey: Record<
          string,
          {
            ...
            description: string
            serials: number[]
            members: Array<{ id: number; serial: number }>
            total_quantity: number
            available_quantity: number
            itemNames: ItemNameRef[]
            _nameIds: Set<number>
        ...
        itemList.forEach((it) => {
          const itemNames: ItemNameRef[] = (it.items_detail ?? []).map((n) => ({
            id: n.id,
            name: n.name,
          }))
          const key =
            groupKeyOf(it.product_description ?? it.description, itemNames) ??
            `ID:${it.id}`
          const g = (byKey[key] ??= {
            key,
            description:
        ...
        const built: Group[] = (Object.values(byKey) as typeof byKey[string][]).map(
          (g) => {
            const rep = [...g.members].sort((a, b) => a.serial - b.serial)[0]
            const memberIds = g.members.map((m) => m.id)
            let splits: SplitRow[] = []
            memberIds.forEach((mid) => {
              if (splitsByItem[mid]) splits = splits.concat(splitsByItem[mid])
            })
            return {
              id: rep.id,
  ...
  const updateGroup = (repId: number, updater: (g: Group) => Group) =>
    setGroups((prev) => prev.map((g) => (g.id === repId ? updater(g) : g)))

  const addSplit = (repId: number) =>
    updateGroup(repId, (g) => ({ ...g, splits: [...g.splits, emptySplit()] }))

  const removeSplit = (repId: number, key: string) =>
    updateGroup(repId, (g) => ({
      ...g,
      splits:
        g.splits.length > 1 ? g.splits.filter((s) => s.key !== key) : g.splits,
    }))

  const changeSplit = (
    repId: number,
    key: string,
    field: 'planned_quantity' | 'unit_price' | 'planned_cif_fc' | 'item_name',
    value: string,
  ) =>
    updateGroup(repId, (g) => {
      const otherQty = g.splits.reduce(
        (s, sp) => s + (sp.key === key ? 0 : num(sp.planned_quantity)),
        0,
      )
      const availLeft = Math.max(g.available_quantity - otherQty, 0)
      const splits = g.splits.map((sp) => {
        if (sp.key !== key) return sp
        let q = num(sp.planned_quantity),
          up = num(sp.unit_price),
          cif = num(sp.planned_cif_fc)
  ...
  const plannedCifTotal = useMemo(
    () =>
      groups.reduce(
        (sum, g) =>
          sum + g.splits.reduce((s, sp) => s + num(sp.planned_cif_fc), 0),
        0,
      ),
    [groups],
  )
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const e = err as { response?: { data?: { error?: string; errors?: unknown } } }
  ...
  return (
    ...
    >
      <Dialog.Portal>
        ...
        <Dialog.Content
          className={cn(
            'fixed left-1/2 top-1/2 z-50 max-h-[90vh] w-full max-w-4xl -translate-x-1/2 -translate-y-1/2',
        ...
        >
          ...
          <div className="mb-3 flex items-center justify-between rounded-md border border-border bg-muted/40 px-3 py-2 text-sm">
            <span>
              Balance CIF: <b>{Number(poolBalance).toFixed(2)}</b>
            </span>
            <span>
              Planned: <b>{plannedCifTotal.toFixed(2)}</b>
            </span>
            <span
              className={
                cifOver ? 'font-semibold text-red-600' : 'font-semibold text-green-600'
              }
            >
                  ...
                  >
                    <div className="mb-2 flex items-center justify-between">
                      ...
                      <div className="text-xs text-muted-foreground">
                        Total {g.total_quantity.toFixed(3)} · Avail{' '}
                        {g.available_quantity.toFixed(3)} ·{' '}
                        <span
                          className={
                            qtyOver ? 'font-semibold text-red-600' : ''
                          }
                        >
