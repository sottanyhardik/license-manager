...
Ported verbatim from legacy/backend/apps/bill_of_entry/parsers/boe_pdf.py.
No business logic changes; only the module path changed.
"""
...
def parse_boe_pdf(source) -> dict[str, Any]:
    ...
    page4 = pages[3] if len(pages) > 3 else ""  # noqa: F841 — reserved for future use
...
def _parse_licences_columnar(page_text):
    ...
    lic_idxs = [i for i, ln in enumerate(lines) if _COL_LIC_NO_RX.match(ln)]
    if not lic_idxs:
        return []
    runs = []
    s = e = lic_idxs[0]
