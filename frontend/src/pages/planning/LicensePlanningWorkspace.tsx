...
export default function LicensePlanningWorkspace() {
    ...
    return <div className="space-y-4 bg-muted/20 p-1" aria-label="SION Planning Workbench">
        ...
        <nav className="sticky top-[65px] z-20 flex items-center gap-2 overflow-x-auto border-y bg-background/95 py-2" aria-label="SION norm tabs" role="tablist" onKeyDown={(event) => { if (!filteredSions.length || !["ArrowLeft", "ArrowRight"].includes(event.key)) return; event.preventDefault(); const index = filteredSions.findIndex((row) => row.id === sion); selectSion(filteredSions[(index + (event.key === "ArrowRight" ? 1 : -1) + filteredSions.length) % filteredSions.length].id); }}>
            ...
            <TabsContent value="rules">
                <section aria-label="Rule workspace" className="grid h-[calc(100dvh-14.5rem)] min-h-[210px] overflow-hidden rounded-lg border bg-card lg:grid-cols-[minmax(320px,38%)_minmax(0,62%)]">
                    ...
                    <section aria-label={draft ? "Rule editor" : "Rule detail"} className="flex min-h-0 flex-col">
                        ...
                        </> : selectedRule ? <div className="space-y-6 p-5">
