# license/views/active_licenses_report.py
"""
Active Licenses Report.

Ported exactly from:
  legacy/backend/apps/license/views/active_licenses_report.py

Shows active licenses with expiry >= (today - N days). Supports JSON
and Excel output (separate sheets per SION norm).

Field mapping identical to expiring_licenses_report.py — see that module.
"""
...
from apps.license.views.expiring_licenses_report import (
    _ACTIVE_STATUS_CODES,
    DEC_0,
    DEC_000,
    _get_balance_cif,
    _get_condition_sheet,
    _get_items_with_balances,
)

# ---------------------------------------------------------------------------
# Helpers specific to active report (days_until_expiry vs days_to_expiry)
# ---------------------------------------------------------------------------
...
def _build_active_license_data(license_obj: LicenseDetailsModel) -> Dict[str, Any]:
    """Build full license dict for one active license (uses days_until_expiry)."""
    days_until_expiry = (
...
def _generate_active_report(days: int = 30, sion_norm: str = None) -> Dict[str, Any]:
    """
    Generate active licenses report.
    Shows all active licenses with expiry from (today - N days) onward.
    """
    today = date.today()
    ...
    for license_obj in licenses:
        ...
        if balance < Decimal("100.00"):
            continue
        license_data = _build_active_license_data(license_obj)
...
def _export_active_to_excel(report_data: Dict[str, Any], days: int) -> HttpResponse:
    """Export active licenses report to Excel. Ported from legacy verbatim."""
    import openpyxl
    ...
    for ld in report_data["licenses"]:
        norms = ld["sion_norms"]
        norm_key = norms[0] if norms else "No Norm"
        licenses_by_norm.setdefault(norm_key, []).append(ld)

    if not licenses_by_norm:
        ws = workbook.create_sheet(title="No Data")
        ws["A1"] = "No active licenses found for the specified criteria"
        ...
        response["Content-Disposition"] = (
            f'attachment; filename="active_licenses_{days}_days.xlsx"'
        )
    ...
    for norm_name, licenses_list in sorted(licenses_by_norm.items()):
        ...
        cell = ws[f"A{cur}"]
        cell.value = (
            f"Active Licenses (Expiry from {report_data['report_period']['from_date']}"
            f" onwards) - {norm_name}"
        )
        ...
        period_cell.value = (
            f"Showing active licenses with expiry >= "
            f"{report_data['report_period']['from_date']}"
        )
        period_cell.alignment = Alignment(horizontal="center")
        cur += 2

        for ld in licenses_list:
            days_val = ld["days_until_expiry"]
            days_text = (
                f"{days_val} days"
                if days_val is not None and days_val >= 0
                else f"{abs(days_val)} days ago" if days_val is not None else "N/A"
            )
            ws.merge_cells(f"A{cur}:L{cur}")
            hdr = ws[f"A{cur}"]
            hdr.value = (
                f"License: {ld['license_number']} | "
                f"Expiry: {ld['license_expiry_date']} | {days_text}"
            )
            hdr.font = Font(bold=True, size=11)
            hdr.fill = PatternFill(start_color="D9EAD3", end_color="D9EAD3", fill_type="solid")
            cur += 1
            ...
            for col_num, header in enumerate(item_headers, 1):
                ...
                c.fill = PatternFill(start_color="38761D", end_color="38761D", fill_type="solid")
        ...
        ns.font = Font(bold=True, size=12, color="FFFFFF")
        ns.fill = PatternFill(start_color="38761D", end_color="38761D", fill_type="solid")
        ...
        iws.font = Font(bold=True, size=11, color="FFFFFF")
        iws.fill = PatternFill(start_color="38761D", end_color="38761D", fill_type="solid")
        ...
        for col_num, val in enumerate(grand_row, 1):
            ...
            c.fill = PatternFill(start_color="274E13", end_color="274E13", fill_type="solid")
        cur += 1
...
class ActiveLicensesViewSet(viewsets.ViewSet):
    """
    ViewSet for Active Licenses Report.
    Route: /api/v1/licenses/active-licenses/
    """
    ...
    def list(self, request):
        """
        GET /api/v1/licenses/active-licenses/
        ...
        try:
            report_data = _generate_active_report(days, sion_norm)
        ...
        if output_format == "excel":
            return _export_active_to_excel(report_data, days)
        return Response(report_data)

    @action(detail=False, methods=["get"])
    def export(self, request):
        """GET /api/v1/licenses/active-licenses/export/ — Excel download."""
        days = _safe_int(request.query_params.get("days"), 30)
        sion_norm = request.query_params.get("sion_norm")
