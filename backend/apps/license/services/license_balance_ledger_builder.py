...
class LicenseBalanceLedgerBuilder:
    ...
    def build_financial_ledger(license_obj, alloc_map=None, ext_map=None):
        ...
        # Needed for the Previous Owner Utilisation debit below whenever
        # has_hidden — computed unconditionally (cheap) so it's available
        # before the "Licence Trade (Purchased)" row loop further down,
        # which ALSO still renders its own row unconditionally (see that
        # loop — Purchase is never suppressed, even though this same CIF
        # is also subtracted here; see the docstring immediately below for
        # why both are correct at once).
        previous_owner_purchase_component = LicenseBalanceCalculator.calculate_purchase_credit(license_obj)
        rows = []
        sr = 1

        # 3-way Opening Balance gate, checked in this order:
        ...
        #      + previous_owner_purchase_component. This row answers "how
        #      much of the original licence was no longer available before
        #      we became the owner" — Hidden BOEs (never ours) AND Purchased
        #      CIF (already bought from the previous owner, so no longer
        #      "available" from them either) both left that original pool.
        #      Licence Purchase is STILL a separate, independent financial
        #      event that answers a DIFFERENT question ("how much did OUR
        #      company acquire") — it re-enters the ledger unconditionally
        #      as its own "Licence Trade (Purchased)" credit row below,
        #      giving Purchase a net-zero effect on the running balance
        #      unless something else (typically a matching Sale) also
        #      touches the same CIF. See `LicenseBalanceCalculator.
        #      calculate_opening_balance`'s docstring for the identical
        #      formula, kept in lock-step so the two never diverge.
        #   2. else has_purchase -> unchanged: `running` starts at 0, no row
        #      (a licence with a Purchase trade tells its story from that
        ...
        if has_hidden:
            ...
            previous_owner_utilisation = hidden_total + previous_owner_purchase_component
            ...
            rows.append({
                ...
                'remarks': 'Previous Owner Utilisation (Hidden BOEs + Licence Trade Purchased)',
        ...
        summary = {
            ...
            # `hidden_total + previous_owner_purchase_component` when
            # has_hidden, else 0 — the exact debit rendered as the
            # "Previous Owner Utilisation" row above. Licence Purchase
            # ALSO still appears separately as its own credit (see
            # `total_purchase_credit` below) — see the opening-balance
            # gate's docstring for why both are correct simultaneously.
            'previous_owner_utilisation': (hidden_total + previous_owner_purchase_component) if has_hidden else DEC_0,
    ...
    def build_customs_ledger(license_obj, show_hidden=False):
        ...
        `show_hidden` affects ONLY which rows are returned — it NEVER
        changes any total. Hidden (previous-owner) BOE debit rows are
        ALWAYS fetched (`get_debit_rows(..., include_hidden=True)`,
        unconditionally) and ALWAYS folded into `total_boe_cif`/`running`/
        `computed_balance`: the Customs Ledger is the permanent, literal
        audit trail of "every physical BOE that came through customs
        against this licence, hidden or not" — Available Customs Balance
        is always `Original Licence CIF - Total BOE Utilisation (hidden +
        visible) - Pending Allotments`, full stop. `show_hidden=False`
        (default) simply drops hidden rows from the returned `rows` list
        afterward (each remaining row keeps the SAME `running_balance` it
        would have had with hidden rows shown — the toggle never
        recomputes anything, it only changes what's visible); `show_hidden
        =True` includes them, flagged via `is_hidden`/`hidden_reason`. This
        is deliberately more literal than the Financial Ledger/Balance
        Engine (`calculate_balance`, `calculate_financial_balance`, both of
        which always exclude hidden BOEs) — the two ledgers are EXPECTED to
        diverge by exactly the hidden BOE total, and that gap is the
        reconciliation signal this workspace surfaces (see this docstring's
        opening paragraphs), never silently forced to match. The Final
        ...
        boe_rows = list(
            ...
            LicenseBalanceCalculator.get_debit_rows(license_obj, include_hidden=True)
            .select_related('bill_of_entry__company', 'sr_number')
            .prefetch_related('sr_number__items')
        ...
        # Visibility-only filter, applied LAST: every total/running_balance
        # above is already computed from the full (hidden-inclusive) row
        # set, so dropping hidden rows here can never change a number that
        # survives into `summary` or any remaining row's `running_balance`
        # — exactly "toggle affects only visibility" (see docstring).
        if not show_hidden:
            rows = [r for r in rows if not r.get('is_hidden')]
        for display_sr, r in enumerate(rows, start=1):
            r['sr'] = display_sr
