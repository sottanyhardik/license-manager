    ...
    def get(self, request, data):
        ...
        for row in ShippingDetails.objects.filter(file_id=data):
            if ShippingDetails.objects.filter(file_id=data, shipping_bill=row.shipping_bill).count() > 1:
                row.delete()
        shipping_bill_details = ShippingDetails.objects.filter(file_id=data)
...
class ShippingBillFetchView(FormView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        status = True
        while status:
            status = fetch_data_to_model(cookies, csrftoken, data_dict, kwargs, captcha, request=self.request)
        if ShippingDetails.objects.filter(scroll_details=False, file_id=kwargs.get('data')).exclude(failed=5, port_of_discharge=None).exists():
