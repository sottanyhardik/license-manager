...
def test_projection_preserves_the_supplied_legacy_value_for_null_and_false():
    ...
    for raw_override in (None, False):
        projection = project_effective_item_cif(
            licence=SimpleNamespace(individual_item_cif_override=raw_override),
            item=item,
            legacy_row_balance=lambda: Decimal("47.50"),
        )
        assert projection.effective_mode == LEGACY
        assert projection.effective_row_balance == Decimal("47.50")
        assert projection.balance_source == "LEGACY"


def test_projection_uses_the_canonical_item_balance_only_when_enabled():
    projection = project_effective_item_cif(
        licence=SimpleNamespace(individual_item_cif_override=True),
