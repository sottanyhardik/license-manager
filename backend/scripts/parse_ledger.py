"""
DGFT ICEGATE Item-Wise Ledger parser (CSV format).

Ported from legacy/backend/scripts/parse_ledger.py — business logic is
IDENTICAL.  Only import paths have been updated to match the new backend's
app layout.

Significant differences from the legacy version:
  - Core masters imported from apps.core.models (the package __init__ re-exports
    CompanyModel, PortModel, SchemeCode, NotificationNumber).
  - Balance is recomputed via apps.license.services.balance_service.recompute_license_balance()
    instead of the legacy update_balance_values() helper.
  - The license app has no signals.py; the only signals on LicenseImportItemsModel
    that need muting during bulk operations are those defined in the app itself.
    We guard defensively — if a signal is not connected, disconnect() is a no-op.
  - BOE signals use the dispatch_uid strings registered in bill_of_entry/models.py.
"""
...
def bulk_get_or_create_license_items(type_credit_list, license, skip_signals=False):
    ...
    with transaction.atomic():
        ...
        # In the new backend there is no signals.py in the license app — the
        # LicenseImportItemsModel has no post_save signals registered via
        # connect().  We still call disconnect() defensively; it is a no-op
        # when the receiver is not connected.
        if skip_signals:
            pass  # No LicenseImportItemsModel post_save signals to mute in new backend.
...
def bulk_get_or_create_boe(boe_row, skip_signals=False):
    ...
    """
    from django.db.models.signals import post_save, post_delete
    from apps.bill_of_entry.models import (
        update_stock,
        delete_stock,
        recalc_exchange_rate_on_row_save,
        recalc_exchange_rate_on_row_delete,
    )
    ...
    with transaction.atomic():
        ...
        if skip_signals:
            # The new backend registers BOE row signals with dispatch_uid strings.
            post_save.disconnect(update_stock, sender=RowDetails, dispatch_uid="boe_update_stock_on_save")
            post_save.disconnect(recalc_exchange_rate_on_row_save, sender=RowDetails, dispatch_uid="boe_recalc_exchange_rate_on_save")
            post_delete.disconnect(delete_stock, sender=RowDetails, dispatch_uid="boe_delete_stock_on_delete")
            post_delete.disconnect(recalc_exchange_rate_on_row_delete, sender=RowDetails, dispatch_uid="boe_recalc_exchange_rate_on_delete")
        ...
        try:
            ...
            if skip_signals:
                post_save.connect(update_stock, sender=RowDetails, dispatch_uid="boe_update_stock_on_save")
                post_save.connect(recalc_exchange_rate_on_row_save, sender=RowDetails, dispatch_uid="boe_recalc_exchange_rate_on_save")
                post_delete.connect(delete_stock, sender=RowDetails, dispatch_uid="boe_delete_stock_on_delete")
                post_delete.connect(recalc_exchange_rate_on_row_delete, sender=RowDetails, dispatch_uid="boe_recalc_exchange_rate_on_delete")
...
def _create_object_inner(data_dict):
    ...
    if flagged:
        logger.warning(
            "Ledger upload: %d row(s) not found in ledger — flagged as dispute for %s",
            flagged, license.license_number,
        )
    if resolved:
        logger.info(
            "Ledger upload: %d dispute(s) resolved for %s",
    ...
    # Trigger balance recomputation for this license.
    # The new backend uses balance_service.recompute_license_balance() instead of
    # the legacy update_balance_values() loop over import items.
    from apps.license.services.balance_service import recompute_license_balance
    recompute_license_balance(license.pk)
