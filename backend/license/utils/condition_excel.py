...
The colours match the frontend ``ConditionBadge`` component so a user looking
at the browser badge and the downloaded Excel sees the same visual cue.
"""
...
# Empty / unknown types return None so callers can branch on "no condition".
_FILLS = {
...
def condition_fill(condition_type: str | None) -> PatternFill | None:
    if not condition_type:
        return None
    colors = _FILLS.get(condition_type)
    if not colors:
        # Unknown value — use a neutral grey rather than nothing so the user
        # still sees that there *is* a condition flagged.
        bg = "E5E7EB"
    else:
        bg = colors[0]
...
def condition_font(condition_type: str | None) -> Font | None:
    if not condition_type:
        return None
    colors = _FILLS.get(condition_type)
    if not colors:
...
def annotate_cell(cell, condition_type: str | None) -> None:
