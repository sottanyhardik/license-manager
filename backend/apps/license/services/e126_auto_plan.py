...
Pipeline:
  ...
  2. Build a records list keyed by import_item.id, using available_quantity
     as the planning quantity — for EVERY category, including the PKO/
     Olive-Oil split target (the split is 40%/60% of the item's CURRENT
     available quantity — see e126_plan.py's module docstring — but ONLY the
  ...
  4. PKO/Olive-Oil balance tracking (business rule: once generated, a split-
     eligible item's split becomes a FIXED commitment — Auto-Plan must
     ...
     eligible, check for EXISTING PKO/Olive-Oil `LicenseItemPlan` rows: if
     ...
     `compute_e126_auto_plan`'s inline comment for why partial overriding
     ...
     same as every other E126 category.
...
shared import item's `available_quantity`, which cannot tell PKO and Olive
Oil apart. This file's job is only to make sure Auto-Plan doesn't stomp on
that independently-draining balance once it exists.
...
# no cross-target attribution ambiguity (a single item_name per import
# item), so it's always safe (and desired) to keep recomputing it fresh from
...
def _existing_split_balances(license_obj, import_item_ids: list[int]) -> dict:
    ...
    Read from this licence's CURRENT PKO/Olive-Oil `LicenseItemPlan` rows —
...
def compute_e126_auto_plan(license_obj) -> tuple[list[dict], float]:
    ...
    # ── Build records for the E126 classifier ────────────────────────────────
    # Use available_quantity (not the original import quantity) as the
    # planning qty for EVERY category — including the PKO/Olive-Oil 40/60
    # split target.
    records: list[dict] = []
