...
``ledger-detail`` and ``export``. Ledger visibility is role based: accounts
migration 0004 deliberately removed the retired ``User.company`` relationship.
Company remains a reporting filter, not an account-level ownership boundary.
"""
...
class LicenseLedgerSecurityTestCase(APITestCase):
    def setUp(self):
        ...
        self.user_a = self._user("user_a")
        self.user_without_company = self._user("no_company")
        self.user_without_role = User.objects.create_user(
            username="no_role", password="testpass123",
        )
    ...
    def test_collection_includes_all_trade_linked_licences_for_ledger_viewer(self):
    ...
    def test_summary_contains_all_trade_linked_licences_for_ledger_viewer(self):
        response = self._get_as(self.user_a, "/api/license-ledger/summary/")
        assert response.status_code == status.HTTP_200_OK
        assert response.data["dfia"]["total_licenses"] == 2
        assert response.data["incentive"]["total_licenses"] == 0

    def test_detail_allows_role_authorized_ledger_viewer(self):
    ...
    def test_export_allows_role_authorized_ledger_viewer(self):
    ...
    def test_company_filter_is_available_to_ledger_viewers_and_validated(self):
        foreign = self._get_as(
    ...
    def test_authentication_and_role_are_required(self):
        ...
        assert self._get_as(self.user_without_company, "/api/license-ledger/").status_code == status.HTTP_200_OK
