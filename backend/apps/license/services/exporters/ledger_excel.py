...
def generate_ledger_detailed_excel(licenses_data, query_params):
    ...
    try:
        ...
        for lic_data in licenses_data:
            ...
            if transactions:
                # Headers (NEW ORDER: Date, Particulars, Type, Items, Sale($), Purchase($), Sale Bill(₹), Purchase Bill(₹), Balance($), P/L(₹), Purchase Bill, SION)
                txn_headers = [
                    'Date', 'Particulars', 'Type', 'Items',
                    'Sale ($)', 'Purchase ($)', 'Sale Bill (₹)', 'Purchase Bill (₹)',
                ...
                for txn in transactions:
                    ...
                    for col_num, value in enumerate(values, 1):
                        ...
                        # Format numeric columns (5-10 are numeric: Sale($), Purchase($), Sale Bill, Purchase Bill, Balance, P/L)
            ...
            ws.column_dimensions['E'].width = 14  # Sale ($)
            ws.column_dimensions['F'].width = 14  # Purchase ($)
