...
describe("SION planning request payloads", () => {
    ...
    it("omits license_ids for the normal SION-first plan and preview", async () => {
        ...
        expect(api.post).toHaveBeenNthCalledWith(1, "sion-planning-rules/plan-sion/", { sion_id: 1 });
        expect(api.post).toHaveBeenNthCalledWith(2, "sion-planning-rules/preview-sion/", { sion_id: 1 });
    });

    it("includes an explicit non-empty license restriction", async () => {
        await planSavedSionRules(1, [10, 20]);
