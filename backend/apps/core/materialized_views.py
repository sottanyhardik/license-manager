...
LEFT JOIN license_licenseflags lf
    ON lf.license_id = ld.id
...
GROUP BY ld.id, ld.license_number, ld.exporter_id,
         ld.license_date, ld.license_expiry_date, lf.is_active;
...
SELECT
    ...
    (SELECT COUNT(*) FROM license_licensedetailsmodel ld
     JOIN license_licenseflags lf ON lf.license_id = ld.id
     WHERE lf.is_active = true) as active_licenses_count,

    (SELECT COUNT(*) FROM license_licensedetailsmodel ld
     JOIN license_licenseflags lf ON lf.license_id = ld.id
     WHERE lf.is_active = true
     AND ld.license_expiry_date < CURRENT_DATE) as expired_licenses_count,

    (SELECT COUNT(*) FROM license_licensedetailsmodel ld
     JOIN license_licenseflags lf ON lf.license_id = ld.id
     WHERE lf.is_active = true
     AND ld.license_expiry_date BETWEEN CURRENT_DATE AND CURRENT_DATE + INTERVAL '30 days')
    ...
    (SELECT COUNT(DISTINCT ld.exporter_id) FROM license_licensedetailsmodel ld
     JOIN license_licenseflags lf ON lf.license_id = ld.id
     WHERE lf.is_active = true) as active_companies_count,
