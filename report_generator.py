...
with open('Book14.csv', newline='') as csvfile:
    ...
    for row in reader:
        ...
        doc.save(file_name)
        # my_docx_folder(file_name, file_name.replace('.docx', '.pdf'))
        doc = DocxTemplate("DOCUMENTS.docx")
        context = {'name': row['name'], 'patient_id': row['patient_id'], 'qr_id': row['qr_id'], 'time_1': row['time_1'],'time_3': row['time_3'], 'time_2': row['time_2'], 'lab_address_1':row['lab_address_1'], 'lab_address_2':row['lab_address_2'],'address_line_1':row['address_line_1'],'address_line_2':row['address_line_2']}
        doc.render(context)
