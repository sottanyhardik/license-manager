...
SchemeCodeViewSet = MasterViewSet.create_viewset(
    SchemeCode,
    SchemeCodeSerializer,
    config={
        "search": ["code", "label"],
        ...
        "form_fields": ["code", "label"],
        "ordering": ["code"],
        "field_meta": {
            "code": {"label": "Code", "type": "text", "required": True, "max_length": 10},
            "label": {"label": "Label", "type": "text", "required": True, "max_length": 100},
        },
    },
)
# Look up by the `code` field rather than PK so URLs like
# /api/masters/scheme-codes/26/ resolve (matches the pre-FK contract where the
# license response carries the code string, not the PK).
SchemeCodeViewSet.lookup_field = "code"
SchemeCodeViewSet.lookup_value_regex = "[^/]+"


NotificationNumberViewSet = MasterViewSet.create_viewset(
    NotificationNumber,
    NotificationNumberSerializer,
    config={
        "search": ["code", "label"],
...
NotificationNumberViewSet.lookup_field = "code"
# Allow slashes in the code (e.g. "025/2023") via URL-encoded `%2F`.
NotificationNumberViewSet.lookup_value_regex = "[^/]+"
