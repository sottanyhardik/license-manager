# license/management/commands/link_pp_norm_items.py
from django.core.management.base import BaseCommand, CommandError
from django.db.models import Count

from apps.license.models import LicenseDetailsModel, LicenseImportItemsModel
from apps.license.utils.item_matcher import bulk_auto_link_license_items, classify_pp_norm_item


class Command(BaseCommand):
    help = (
        "Run ItemNameModel auto-linking for every licence carrying the 'PP' SION "
        "norm class, applying the PP-norm packaging rules (PP / HDPE / LDPE / "
        "Paper / Paper Board — see item_matcher.classify_pp_norm_item) ahead of "
        "the generic matcher. Use this to backfill existing PP-norm licences "
        "after the rules were added; new licences pick them up automatically "
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            ...
            action="store_true",
            help=(
                "Report which import items the PP-norm rules would match, "
                "without writing any changes. Does not create ItemNameModel "
                "rows or preview the generic (non-PP-rule) matcher — pair "
                "with `populate_license_items --dry-run` for that."
            ),
        )
        parser.add_argument(
            "--clear",
    ...
    def handle(self, *args, **opts):
        ...
        licenses = (
            ...
            .filter(export_license__norm_class__norm_class="PP")
            .distinct()
        ...
        if license_number:
            ...
            if not licenses.exists():
                raise CommandError(f"No licence found with license_number={license_number!r} and norm 'PP'")

        total = licenses.count()
        self.stdout.write("=" * 80)
        self.stdout.write(f"PP-norm item linking — {total} licence(s) found")
        self.stdout.write(f"Dry run: {dry_run}   Clear existing: {clear_existing}")
        self.stdout.write("=" * 80)

        if total == 0:
            self.stdout.write(self.style.WARNING("No licences found with SION norm class 'PP'."))
            return
    ...
    def _preview(self, licenses, *, clear_existing):
        """Read-only preview using classify_pp_norm_item directly (no
        ...
        otherwise only currently-unlinked import items are."""
        total_would_match = 0
        for license_obj in licenses.iterator():
            ...
            for ii in import_items:
                ...
                result = classify_pp_norm_item(hs_code_str, ii.description)
                if not result:
                    continue
                name, rule_tag = result
                total_would_match += 1
                self.stdout.write(
        ...
        self.stdout.write("")
        self.stdout.write(self.style.SUCCESS(
            f"Would match {total_would_match} import item(s) via PP-norm rules "
