"""
Planned Report - List LicenseItemPlan rows (utilization-plan-driven) with filters.

Split out of item_report.py so that the Item Report can go back to being
rooted on LicenseImportItemsModel (every import item, plan or no plan) while
this view keeps the plan-rooted behavior for users who specifically want to
report on planned/split utilization data.
"""
...
from apps.license.views.item_report import _safe_int, _ExcelPassthroughRenderer


logger = logging.getLogger(__name__)


class PlannedReportView(APIView):
    """
    Report showing LicenseItemPlan rows (planned/split utilization data) with filters.

    GET parameters:
        - item_names: Comma-separated item name IDs for filtering (multiselect) — matches the
          PLANNED item name (LicenseItemPlan.item_name), not the import item's own M2M tag.
    ...
    def export_to_excel(self, item_names=None, company_ids=None, exclude_company_ids=None, min_balance=200, min_avail_qty=0, license_status='active', is_restricted=None, purchase_status=None, product_description=None, hsn_code=None, norms=None, notification_numbers=None, expiry_date_from=None, expiry_date_to=None):
        ...
        response['Content-Disposition'] = 'attachment; filename=planned_report.xlsx'
        return response


class PlannedReportViewSet(viewsets.ViewSet):
    """
    ViewSet for Planned Report actions like getting available items for filter.
    """
    permission_classes = [ReportPermission]
    ...
    def available_items(self, request):
        ...
        Showing import-side item names here would be misleading because this
        report is driven by the plan, not the import M2M tags.
