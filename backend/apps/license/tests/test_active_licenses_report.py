"""
Tests for the Active Licenses Report (backend/apps/license/views/active_licenses_report.py).

Phase 2A regression coverage — includes the permanent regression test for a
real bug found while writing these tests: this view had no `renderer_classes`
override, so DRF's content negotiation raised `Http404` for `?format=excel`
BEFORE `get()` ever ran (see `rest_framework.negotiation.
DefaultContentNegotiation.filter_renderers` — it raises `Http404`, not
`NotAcceptable`, when no renderer's `.format` matches the query param).
Fixed by registering `apps.core.reports.renderers.ExcelPassthroughRenderer`,
the same pattern `ItemReportView`/`LicensePurchaseProfitReportView` already
used. `test_excel_export_returns_valid_workbook_matching_json` below is the
...
REPORT_URL = "/api/reports/active-licenses/"


@pytest.fixture
def report_viewer_client(db):
    user = User.objects.create_user(
        username="active-licenses-viewer",
        email="active-licenses-viewer@example.com",
...
@pytest.fixture
def al_masters(db):
    head_norm = HeadSIONNormsModel.objects.create(name="Active Licenses Test Head Norm")
    return {
        "exporter": CompanyModel.objects.create(iec="4440001111", name="Active Licenses Exporter"),
        "hs_code": HSCodeModel.objects.create(hs_code="55556666", product_description="Active Licenses Test Product"),
        "item_a": ItemNameModel.objects.create(name="Active Licenses Item A"),
...
def _make_license(number, exporter, purchase_status=None, *, expiry_days=30):
...
def test_json_response_matches_envelope_shape(report_viewer_client, al_masters):
    lic = _make_license("ACT-LIC-001", al_masters["exporter"], al_masters["ge_status"])
    _make_export_item(lic, al_masters["e1_norm"])
    _make_import_item(lic, al_masters["hs_code"], [al_masters["item_a"]])
    ...
    assert data["licenses"][0]["license_number"] == "ACT-LIC-001"


@pytest.mark.django_db
def test_excel_export_returns_valid_workbook_matching_json(report_viewer_client, al_masters):
    ...
    lic = _make_license("ACT-LIC-010", al_masters["exporter"], al_masters["ge_status"])
    _make_export_item(lic, al_masters["e1_norm"])
    _make_import_item(lic, al_masters["hs_code"], [al_masters["item_a"]])
    ...
    header_row = next(
        ...
        and sheet.cell(row=r, column=1).value.startswith("License: ACT-LIC-010")
    )
    # Layout: header row, then 4 detail rows (Notification/License Date,
    # Exporter/Port, SION Norms/Balance CIF, Ledger Date) — see
    # ActiveLicensesReportView.export_to_excel's `details` list.
...
@pytest.mark.django_db
def test_export_filename_follows_standard_convention(report_viewer_client, al_masters):
    lic = _make_license("ACT-LIC-020", al_masters["exporter"], al_masters["ge_status"])
    _make_export_item(lic, al_masters["e1_norm"])
    _make_import_item(lic, al_masters["hs_code"], [al_masters["item_a"]])

    response = report_viewer_client.get(REPORT_URL, {"days": 30, "format": "excel"})
    disposition = response["Content-Disposition"]
    assert disposition.startswith('attachment; filename="active-licenses_')
