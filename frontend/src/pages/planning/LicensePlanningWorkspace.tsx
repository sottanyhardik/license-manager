...
export default function LicensePlanningWorkspace() {
    ...
    const planSion = async (mode: SionPlanningMode) => { if (busy || !sion || !canExecuteSion) return; const restore = preserveScroll(); setConfirmForceAll(false); setBusy(mode === "ALL" ? "plan-all" : "plan-new"); setError(""); try { await planSavedSionRules(sion, mode); setPreview(await previewSavedSionRules(sion, mode)); setWorkbenchTab("simulation"); toast.success(mode === "ALL" ? `${sionLabel} full eligible universe reprocessed` : `${sionLabel} new eligible data planned`); setRules(await fetchSionPlanningRules(sion)); } catch { toast.error("Planning failed"); } finally { setBusy(null); restore(); } };
    ...
    return <div className="planning-workbench space-y-4 rounded-xl bg-muted/20 p-1" aria-label="SION Planning Workbench">
        <div className="sticky top-0 z-30 -mx-2 rounded-xl border border-border/70 bg-background/95 px-3 py-3 shadow-sm backdrop-blur">
            <div className="flex flex-wrap items-center gap-3">
                ...
                <div className="mr-auto"><h1 className="text-xl font-semibold">SION Planning Workbench</h1><p className="text-sm text-muted-foreground">Configure, validate, simulate and publish planning rules</p></div>
        ...
        <nav className="sticky top-[76px] z-20 flex items-center gap-2 overflow-x-auto rounded-xl border border-border/70 bg-background/95 px-2 py-2 shadow-sm backdrop-blur" aria-label="SION norm tabs" role="tablist" onKeyDown={(event) => { if (!filteredSions.length || !["ArrowLeft", "ArrowRight"].includes(event.key)) return; event.preventDefault(); const index = filteredSions.findIndex((row) => row.id === sion); selectSion(filteredSions[(index + (event.key === "ArrowRight" ? 1 : -1) + filteredSions.length) % filteredSions.length].id); }}>
        ...
        <main id="sion-workspace" role="tabpanel" aria-labelledby={sion ? `sion-tab-${sion}` : undefined} className="min-w-0 space-y-3">
          ...
          {!sion && !loading ? <section className="rounded-xl border bg-card p-10 text-center"><ClipboardList className="mx-auto size-8 text-muted-foreground" /><h2 className="mt-3 font-semibold">Select a SION norm</h2><p className="mt-1 text-sm text-muted-foreground">Choose a norm from the left panel to review its rules, validation and version history.</p><Button className="mt-4" variant="outline" onClick={() => filteredSions[0] && selectSion(filteredSions[0].id)} disabled={!filteredSions.length}>Select first active norm</Button></section> : sion && <>
          <section className="rounded-xl border border-border/80 bg-card p-4 shadow-md shadow-primary/5"><div className="flex flex-wrap items-start justify-between gap-3"><div><p className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">Selected norm</p><h2 className="text-2xl font-semibold">{sionLabel}</h2><p className="text-sm text-muted-foreground">{selectedSion?.description ?? "Planning configuration"}</p></div><div className="text-right text-xs text-muted-foreground">Version {Math.max(0, ...rules.map((r) => r.version ?? 0)) || "Not available"}<br />Last updated: {displayDate(selectedRule?.modified_on)}</div></div><div className="mt-4 grid grid-cols-2 gap-2 sm:grid-cols-3 xl:grid-cols-6">{[["Total Rules", rules.length], ["Active Rules", activeSavedRules.length], ["Draft Changes", hasUnsavedChanges ? 1 : 0], ["Validation Issues", validationIssueCount], ["Matched Licences", preview?.summary?.licenses_matched ?? "Not calculated"], ["Last Simulation", preview ? "Available" : "Not run"]].map(([label, value]) => <button key={String(label)} type="button" onClick={() => { if (label === "Active Rules") setStatusFilter("active"); if (label === "Validation Issues") setStatusFilter("active"); if (label === "Matched Licences" || label === "Last Simulation") setWorkbenchTab("simulation"); }} className="rounded-lg border border-border/70 bg-muted/30 p-2.5 text-left shadow-sm transition-colors hover:bg-primary/5"><div className="text-[10px] font-semibold uppercase text-muted-foreground">{label}</div><div className="mt-1 text-lg font-semibold tabular-nums">{value}</div></button>)}</div></section>
