"""
E126 Auto-Plan service.

Delegates classification and allocation entirely to the existing, tested
e126_plan engine (classify_e126_record / plan_e126_per_item_split), then
converts the result into ready-to-save LicenseItemPlan line dicts. Mirrors
e132_auto_plan.py exactly (PKO/Olive-Oil replacing PKO/Cheese).
...
Pipeline:
  ...
  2. Build a records list keyed by the GROUP's representative import item id
     ...
     planning quantity — for EVERY category, including the PKO/Olive-Oil
     split target (the split is 50%/50% of the group's CURRENT total
     available quantity — see e126_plan.py's module docstring — but ONLY
     the first time it's generated; see step 4).
  3. Call plan_e126_per_item_split(records, balance_cif) — this runs the
     E126 waterfall (classify, allocate, wastage-reduction rebalance).
  4. PKO/Olive-Oil balance tracking (business rule: once generated, a split-
     eligible group's split becomes a FIXED commitment — Auto-Plan must
     never regenerate or recalculate it from the current available_quantity
     again). For any group the engine still classifies as split-eligible,
     check for EXISTING PKO/Olive-Oil `LicenseItemPlan` rows across EVERY
     member of the group (`plan_grouping.existing_split_balances_for_groups`
     — not just its current representative, since a split generated before
     a re-serialization may still be sitting on a now-non-representative
     ...
     unchanged instead of the engine's freshly (re)computed 50/50 split
     (never just one side — see `compute_e126_auto_plan`'s inline comment
     ...
     pure recompute from `available_quantity`, same as every other E126
     category.
...
import item's `available_quantity`, which cannot tell PKO and Olive Oil
...
SCOPE for this file — not implemented here, to be specified separately.
...
from apps.license.services.e126_plan import OLIVE_OIL, PKO, PLANNING_ORDER, UNIT_PRICE, plan_e126_per_item_split
...
# Only these two planning items are ever "fixed once generated" — Nuts has
# no cross-target attribution ambiguity (a single item_name per group), so
# it's always safe (and desired) to keep recomputing it fresh from
# available_quantity on every run.
_SPLIT_TARGET_NAMES = (PKO, OLIVE_OIL)
...
def compute_e126_auto_plan(license_obj) -> tuple[list[dict], float]:
    """
    Run the E126 Auto Plan for *license_obj*.
    ...
    # ── Pre-fetch ItemNameModel IDs for all E126 planning-item labels ─────────
    ...
    # ── Build records for the E126 classifier — one per GROUP ─────────────────
    ...
    # category — including the PKO/Olive-Oil 50/50 split target.
    records: list[dict] = []
    ...
    # ── Call the E126 waterfall ───────────────────────────────────────────────
    # plan_e126_per_item_split returns:
    ...
    # ── PKO/Olive-Oil balance preservation (module docstring, step 4) ────────
    ...
    for rec in records:
        ...
        if preserved_for_item:
            ...
            # applies to the WHOLE split (PKO + Olive Oil together, since
            # both are always generated in the SAME Auto-Plan run), never one
            ...
            # 50/50 assumption — silently breaking quantity conservation. A
            ...
            for planning_item in _SPLIT_TARGET_NAMES:
                ...
                item_lines.append({
                    ...
                    'note': f"Auto-planned (E126 — {planning_item}) — existing plan preserved",
                })
            ...
            for sp in item_splits:
                ...
                item_lines.append({
                    ...
                    'planned_cif_fc':   cif,
                    'note': (
                        f"Auto-planned (E126 — {planning_item or 'Unclassified'})"
                    ),
                })
                item_planning_items.append(planning_item)
                item_cif += cif
        ...
        if not validate_group_plan_lines(
            ...
            logger.warning(
                "compute_e126_auto_plan: rejecting plan for group represented by "
