...
Shows active licenses with expiry date >= (today - 30 days)
...
class ActiveLicensesReportView(View):
    ...
    def generate_report(self, days: int = 30, sion_norm: str = None) -> Dict[str, Any]:
        ...
        Shows all active licenses with expiry from (today - N days) to the furthest future date.
        ...
        # Get active licenses with expiry >= (today - N days)
        # This includes licenses expiring in the future (2026, 2027, etc.)
    ...
    def _build_license_data(self, license_obj: LicenseDetailsModel) -> Dict[str, Any]:
        ...
        days_until_expiry = (license_obj.license_expiry_date - date.today()).days
    ...
    def export_to_excel(self, report_data: Dict[str, Any], days: int) -> HttpResponse:
        ...
        if not licenses_by_norm:
            ...
            worksheet['A1'] = "No active licenses found for the specified criteria"
        ...
        for norm_name, licenses_list in sorted(licenses_by_norm.items()):
            ...
            # Title
            title = f"Active Licenses (Expiry from {report_data['report_period']['from_date']} onwards) - {norm_name}"
            ...
            # Report period
            period_text = f"Showing active licenses with expiry >= {report_data['report_period']['from_date']}"
            worksheet.merge_cells(f'A{current_row}:L{current_row}')
            ...
            for license_data in licenses_list:
                # License header
                days_text = f"{license_data['days_until_expiry']} days" if license_data['days_until_expiry'] >= 0 else f"{abs(license_data['days_until_expiry'])} days ago"
                license_header = f"License: {license_data['license_number']} | Expiry: {license_data['license_expiry_date']} | {days_text}"
                worksheet.merge_cells(f'A{current_row}:L{current_row}')
                header_cell = worksheet[f'A{current_row}']
                header_cell.value = license_header
                header_cell.font = Font(bold=True, size=11)
                header_cell.fill = PatternFill(start_color='D9EAD3', end_color='D9EAD3', fill_type='solid')
                current_row += 1
                ...
                for col_num, header in enumerate(item_headers, 1):
                    ...
                    cell.fill = PatternFill(start_color='38761D', end_color='38761D', fill_type='solid')
            ...
            norm_summary_cell.font = Font(bold=True, size=12, color='FFFFFF')
            norm_summary_cell.fill = PatternFill(start_color='38761D', end_color='38761D', fill_type='solid')
            ...
            items_summary_cell.font = Font(bold=True, size=11, color='FFFFFF')
            items_summary_cell.fill = PatternFill(start_color='38761D', end_color='38761D', fill_type='solid')
...
class ActiveLicensesViewSet(viewsets.ViewSet):
    ...
    def list(self, request):
        """
        Get active licenses report.

        Query Parameters:
            days: Number of days to look back (default: 30)
            sion_norm: Filter by SION norm (optional)
        """
        view = ActiveLicensesReportView()
        return view.get(request)

    @action(detail=False, methods=['get'])
    def export(self, request):
        """
        Export active licenses to Excel.
