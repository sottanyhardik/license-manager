...
Run with: python manage.py shell < verify_ledger_0310833996.py
Or: from verify_ledger_0310833996 import *
"""
...
try:
    ...
    print(f"  - Exporter: {license.exporter.name if license.exporter else 'N/A'}")
...
# Note: current_balance is in balance_currency (USD for DFIA),
# while total_profit_loss is in INR (bill amounts).
# They measure different things and should NOT be equal.
summary = ledger_dataset.get('summary', {})
balance_currency = summary.get('balance_currency', 'USD')
profit_currency = summary.get('profit_currency', 'INR')

# Verify that profit_loss = total_credit_bill - total_debit_bill
total_credit_bill = summary.get('total_credit_bill', Decimal('0.00'))
total_debit_bill = summary.get('total_debit_bill', Decimal('0.00'))
expected_profit_loss = total_credit_bill - total_debit_bill
actual_profit_loss = summary.get('total_profit_loss', Decimal('0.00'))
if expected_profit_loss != actual_profit_loss:
    issues.append(f"Profit/Loss calculation: expected={expected_profit_loss} but got={actual_profit_loss}")
...
# Check 4b: Verify current_balance = total_credit - total_debit
total_credit = summary.get('total_credit', Decimal('0.00'))
total_debit = summary.get('total_debit', Decimal('0.00'))
current_balance = summary.get('current_balance', Decimal('0.00'))
expected_balance = total_credit - total_debit
if expected_balance != current_balance:
    issues.append(f"Current balance calculation: expected={expected_balance} ({balance_currency}) but got={current_balance}")

# Check 5: Verify profit state is correct based on profit_loss (INR)
profit_loss = summary.get('total_profit_loss', Decimal('0.00'))
if profit_loss > Decimal('0.00') and profit_state != 'PROFIT':
    issues.append(f"Profit state mismatch: profit_loss={profit_loss} INR (positive) but profit_state={profit_state} (not PROFIT)")
elif profit_loss < Decimal('0.00') and profit_state != 'LOSS':
    issues.append(f"Profit state mismatch: profit_loss={profit_loss} INR (negative) but profit_state={profit_state} (not LOSS)")
elif profit_loss == Decimal('0.00') and profit_state != 'BREAK_EVEN':
    issues.append(f"Profit state mismatch: profit_loss={profit_loss} INR (zero) but profit_state={profit_state} (not BREAK_EVEN)")
