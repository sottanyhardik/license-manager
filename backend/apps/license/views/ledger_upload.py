"""
Ledger upload API endpoint for processing DFIA/DGFT ICEGATE ledger files.

Ported from legacy/backend/apps/license/views/ledger_upload.py.

Routes:
    POST /api/v1/licenses/upload-ledger/
    GET  /api/v1/licenses/ledger-task-status/<task_id>/
"""
import csv
import io
import logging
...
from scripts.parse_ledger import create_object, parse_license_data
...
class LedgerUploadView(APIView):
    """
    API endpoint to upload and process ledger CSV/HTM files.
    ...
    throttle_classes = []  # No UploadRateThrottle in new backend — add if needed.
    ...
    def _handle_async(self, files, max_file_size):
        ...
        for uploaded_file in files:
            ...
            try:
                ...
                logger.info("Parsed %d licenses from %s, dispatching tasks", len(dict_list), uploaded_file.name)

                tasks = []
                for dict_data in dict_list:
                    serialized = self._serialize_for_celery(dict_data)
                    task = process_single_license_task.apply_async(args=[serialized], queue='ledger')
                ...
                logger.info("Dispatched %d tasks for %s", len(tasks), uploaded_file.name)
    ...
    def _handle_sync(self, files, max_file_size):
        ...
        for file_sequence_number, uploaded_file in enumerate(files, start=1):
            try:
                logger.info(
                    ...
                    file_sequence_number, len(files), uploaded_file.name,
                )
                ...
                dict_list = self._parse_file(uploaded_file)
                logger.info("Parsed %d license(s) from %s", len(dict_list), uploaded_file.name)
                ...
                for idx, dict_data in enumerate(dict_list, start=1):
                    ...
                    try:
                        ...
                        logger.info("Processed license %d/%d: %s", idx, len(dict_list), license_number)
                    except Exception as license_error:
                        error_msg = str(license_error)
                        failed_licenses.append({'license': license_no, 'error': error_msg})
                        logger.error(
                            "Error creating license %s at index %d: %s",
                            license_no, idx, error_msg, exc_info=True,
                        )
                ...
                logger.info(
                    ...
                    uploaded_file.name, len(created_license_numbers), len(failed_licenses),
                )

            except Exception as e:
                logger.error("Error processing file %s: %s", uploaded_file.name, str(e), exc_info=True)
                all_results.append({
                    'file': uploaded_file.name,
                    'success': False,
                    'error': str(e)
        ...
        response_data = {
            'message': (
                f'Processed {len(total_licenses)} license(s) from '
                f'{len([r for r in all_results if r["success"]])} file(s)'
            ),
