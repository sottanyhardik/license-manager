...
class LicenseFilterSet(BaseFilterSet):
    ...
    def filter_by_status(self, queryset, name, value):
        ...
        if value == 'active':
            ...
            return queryset.filter(Q(flags__is_expired=True) | Q(license_expiry_date__lt=date.today()))
            ...
            return queryset.filter(flags__is_incomplete=True)
    ...
    def filter_has_balance(self, queryset, name, value):
        ...
        if value:
            return queryset.filter(balance__balance_cif__gt=0)
        else:
            return queryset.filter(balance__balance_cif=0)
...
class ItemReportFilterSet(filters.FilterSet):
    ...
    def filter_license_status(self, queryset, name, value):
        ...
        if value == 'active':
            ...
            return queryset.filter(
                Q(license__flags__is_expired=True) |
