...
from apps.bill_of_entry.models import BillOfEntryModel, RowDetails, annotate_and_exclude_hidden
...
def list_boe_rows(license_obj) -> List[Dict[str, Any]]:
    ...
    grouped = (
        annotate_and_exclude_hidden(
            ...
            # trail) are excluded entirely from this BOEs-tab summary —
            # the marker is BOE-level (every row on that physical BOE,
            # regardless of transaction_type), so a hidden BOE simply
            # never appears as a tab entry, matching the Pending BOE rule
            # (previous-owner BOEs must never surface in this workflow).
