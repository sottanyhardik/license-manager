...
class PdfExportAuthorizationTests(LicenseBalanceLedgerFixtureMixin, TestCase):
    """
    Authorization verification for PDF export endpoints.
    Tests role-based access control on the balance_pdf action.
    """
    ...
    def setUp(self):
        ...
        self.user_with_license_viewer = User.objects.create_user(
            username='license_viewer',
            email='viewer@test.com',
            password='testpass123'
        )
        self.user_with_license_viewer.groups.add(
            self._get_or_create_group('LICENSE_VIEWER')
        )

        self.user_with_license_manager = User.objects.create_user(
            username='license_manager',
            email='manager@test.com',
            password='testpass123'
        )
        self.user_with_license_manager.groups.add(
            self._get_or_create_group('LICENSE_MANAGER')
        )
        ...
        from apps.license.models import LicenseExportItemModel
        LicenseExportItemModel.objects.create(license=self.license, cif_fc=Decimal("5000.00"))
    ...
    def test_authorized_license_viewer_can_access_pdf(self):
        """User with LICENSE_VIEWER role can access PDF export."""
        self.api_client.force_authenticate(user=self.user_with_license_viewer)
        response = self.api_client.get(f'/api/licenses/{self.license.id}/balance-pdf/')
        self.assertIn(response.status_code, [200, 404], f"Unexpected status: {response.status_code}")
        if response.status_code == 200:
            self.assertEqual(response['Content-Type'], 'application/pdf')

    def test_authorized_license_manager_can_access_pdf(self):
        """User with LICENSE_MANAGER role can access PDF export."""
        self.api_client.force_authenticate(user=self.user_with_license_manager)
        response = self.api_client.get(f'/api/licenses/{self.license.id}/balance-pdf/')
        self.assertIn(response.status_code, [200, 404], f"Unexpected status: {response.status_code}")
        if response.status_code == 200:
            self.assertEqual(response['Content-Type'], 'application/pdf')
    ...
    def test_unauthorized_user_gets_403(self):
        """User without proper roles gets 403 Forbidden."""
        self.api_client.force_authenticate(user=self.user_without_role)
        response = self.api_client.get(f'/api/licenses/{self.license.id}/balance-pdf/')
        self.assertEqual(response.status_code, 403)

    def test_unauthenticated_user_gets_401_or_403(self):
        """Unauthenticated user gets 401 or 403 Unauthorized."""
        response = self.api_client.get(f'/api/licenses/{self.license.id}/balance-pdf/')
        self.assertIn(response.status_code, [401, 403])

    def test_nonexistent_license_returns_404(self):
        """Request for non-existent license returns 404."""
        self.api_client.force_authenticate(user=self.user_with_license_viewer)
        response = self.api_client.get('/api/licenses/999999/balance-pdf/')
